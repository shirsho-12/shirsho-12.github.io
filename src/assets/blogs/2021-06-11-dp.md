...
under an optimal policy must equal the expected return for the best action from that state. At a state `s`, we choose the action `a` that gives the maximum reward $R_{t %2B 1}$ plus the discounted reward from the next state $\gamma V_{*}(S_{t %2B 1})$.
